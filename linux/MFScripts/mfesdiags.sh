...
# 27Apr2017 GMC 6.2j Corrected type in tmp directory for mfds export
# 27Apr2017 DAVS 6.3 Added h option to tar to follow symlinks
#                    trz file will now embed region name and date time in name
#                    collect MFDBFH config file if available.
# 08Dec2022 PMW      Harvest tags of any overlays in the installed product
# 17May2023 MAP 6.3b Collect dbfhadmin -diags 
# 21May2023 MAP 6.3c Collect dbfhadmin -diags only if PAC vars set - cosmetic change
#                  
################################################################################
...
tar cfh $WRKDIR/mfds.tar -C $COBDIR/etc/ mfds >/dev/null 2>&1
...
#Get MFDBFH_CONFIG file
logit 2 Checking for MFDBFH_CONFIG config file
MFDBFH_CONFIG_SET=`getEnvFromFile MFDBFH_CONFIG 13 $MDUMP_ENV_FILE`
logit 3 MFDBFH_CONFIG_SET=$MFDBFH_CONFIG_SET
if [ "$MFDBFH_CONFIG_SET" ]
then
  MFDBFH_CONFIG=$MFDBFH_CONFIG_SET
  NEW_PATH=
  CheckForDollar $MFDBFH_CONFIG
  MFDBFH_CONFIG=$NEW_PATH
  logit 2 "Copying MFDBFH config file"
  logit 3 cp -p $MFDBFH_CONFIG $WRKDIR
  cp -p $MFDBFH_CONFIG $WRKDIR
  RET=$?
  if [ $RET -ne 0 ]; then
    logit 2 Unable to copy MFDBFH trace config file from $MFDBFH_CONFIG [ret=$RET]
    COLLECTCTF=0
  fi            
else
  logit 3 MFDBFH_CONFIG not set in region - check OS
  if [ "$MFDBFH_CONFIG" ]; then
    logit 2 "Copying CTF trace config file"
    logit 3 cp -p $MFDBFH_CONFIG $WRKDIR
    cp -p $MFDBFH_CONFIG $WRKDIR
    RET=$?
    if [ $RET -ne 0 ]; then
      logit 2 Unable to copy CTF trace config file from $MFDBFH_CONFIG [ret=$RET]
      COLLECTCTF=0
    fi          
  else
    logit 3 "MFDBFH_CONFIG not set in region or OS - no CTF trace"
    COLLECTCTF=0
  fi
fi


#Get MFDBFH -diags collection
logit 3 Collecting MFDBFH -diags
echo do dbfhadmin
COLLECTdbfhadmindiags=1

#Get ES_DB_SERVER variable
logit 2 Checking for ES_DB_SERVER variable
ES_DB_SERVER_SET=`getEnvFromFile ES_DB_SERVER 12 $MDUMP_ENV_FILE`
logit 3 ES_DB_SERVER_SET=$ES_DB_SERVER_SET
if [ "$ES_DB_SERVER_SET" ]
then
  ES_DB_SERVER=$ES_DB_SERVER_SET
else
  logit 3 ES_DB_SERVER not set in region - check OS
  if [ -n "$ES_DB_SERVER" ] 
  then
    ES_DB_SERVER=$ES_DB_SERVER
  else
    logit 3 "ES_DB_SERVER not set in region or OS - no dbfhadmin -diags"
    COLLECTdbfhadmindiags=0
  fi
fi

#Get ES_PAC variable
logit 2 Checking for ES_PAC variable
ES_PAC_SET=`getEnvFromFile ES_PAC 6 $MDUMP_ENV_FILE`
logit 3 ES_PAC_SET=$ES_PAC_SET
if [ "$ES_PAC_SET" ]
then
  ES_PAC=$ES_PAC_SET
else
  logit 3 ES_PAC not set in region - check OS
  if [ -n "$ES_PAC" ] 
  then
    ES_PAC = $ES_PAC
  else
    logit 3 "ES_PAC not set in region or OS - no dbfhadmin -diags"
    COLLECTdbfhadmindiags=0
  fi
fi
...
if [ $COLLECTdbfhadmindiags -eq 1 ] ; then
  dbfhadmin -diags -usedb:$ES_DB_SERVER -name:$ES_PAC
fi
...
logit 3 "Harvest ES overlay binary tags"
export ES_BUILD_TAG=`grep CASCD0099I $SRV_LOC/console.log |gawk '{print $7}'`
for k in lib bin
do 
    for j in dfh cas
    do 
        for i in $COBDIR/$k/$j*
        do 
            export CURR_BIN_TAG=`strings $i | grep "@(#) ES "|gawk '{print $4}'`
            if [ $CURR_BIN_TAG != $ES_BUILD_TAG ] ; then
                echo $i - $CURR_BIN_TAG
            fi
        done
    done
done > $WRKDIR/ESbinaryTags.lst 2>/dev/null
unset ES_BUILD_TAG
unset CURR_BIN_TAG
...
logit 3 "Rename with Date and Time"
mv $OUTDIR/mfesdiags.trz $OUTDIR/mfesdiags-$SRVNAME-`date +%Y%m%d%H%M`.trz

#Tidy up - note can't output to snapshot.log on/after the rm command...
if [ "$REMOVE_TMP_FILES" = "1" ]; then
#DEL_TMP can be set to skip prompt to delete temp files
  case $DEL_TMP in
    y|Y)
      RESP=Y
      ;;
...
# If we are here, the compressed tar operation was successful so tell user to provide this collection
  echo ' '
  echo "*************************************************************************"
  echo "Please send $OUTDIR/mfesdiags-$SRVNAME-`date +%Y%m%d%H%M`.trz to Micro Focus Customer Care"
  echo "*************************************************************************"
