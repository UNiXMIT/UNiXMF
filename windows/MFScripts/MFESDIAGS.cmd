...
CLS
SETLOCAL
set MFESDIAGSVER=3.0
ECHO *** MFESDIAGS.CMD VERSION %MFESDIAGSVER% ***
REM ECHO %DATE% %TIME%
echo.
ECHO *** Copyright © Micro Focus [2010]-[2018]. All rights reserved.
echo.
...
REM                 Added timeout to cScript command - if occurs don't remove SNAPSHOT files.
REM                 Creating .txt files in MFDS_SCHEMA and each CTF sub-dir created to prevent them being empty.
REM  2.9d           Getting tasklist in foreground so we complete before zipping
REM  2.9e 24Jan2018 Collecting sysdir contents if dir exists (not checking for a console.log)
REM  3.0  25Jan2018 Checking if JOURNAL_LOC is blank (mfds -h option not valid in early products, SX5.1, NX6.0)
REM                 CTF: Now copying .log* and .ctb* files to get all 'generations'
REM                 Removing any leading space when region name is entered
REM       24Jun2020 Fixed issue with ZIP not working when run from a folder with a space.
REM    14Feb2020 HCL/UVE Added setting of TEMP to route diagnostics output off of C drive.
REM                 Will be redirected to DIAGTEMP variable location
REM                 Added Call to customdiags.cmd to allow customization
REM                 Increased TIMEOUT of cscript to 180
REM
REM    26Jan2022    Added Support for using USERID/PASSWORD and will prompt for User / Pass if not supplied and secured.
REM                 Added Support for getting MFDBFH_CONFIG file if it exists in region.
REM                 Script can now be run with "mfesdiags REGION USER PASSWORD"
REM                 ESCWA Folder will now also be collected.
REM                 If ZIPTIMEOUT=nnn is set then this will be used as the timeout to cscript.
REM    28Feb2022    Always attempt dump even if region is not responding.
REM    28Jun2022    Resolve issues when run in a Windows Container. cscript does not seem to allow ZIP 
REM                 File creation so switch to a powershell command in a container.
...
REM 
REM    12Nov2020 HCL/UVE Start of Changes
REM  Allow Redirect diagnostic output from TEMP Folder
if NOT "%DIAGTEMP%"=="" (
    set TEMP=%DIAGTEMP% 
)
REM    12Nov2020 HCL/UVE End of Changes
REM 
call :CREATE_SNAPDIR
...
REM Check if Running under Docker.
call :TEE1  Checking if running under Windows Container
sc queryex cexecsvc|Find "STATE"|Find "RUNNING">Nul&&(
    call :TEE1  Running in Windows Container
	set MFES_DOCKER=1
)||(
    call :TEE1  Not running in Windows Container
)
...
REM We will only Convert to Short Path if ZIP is specified.
IF NOT %ZIP%==none (
...
SC queryex LanmanServer|Find "STATE"|Find /v "RUNNING">Nul&&(
    echo LanmanServer not started
)||(
	net share > %SYSINFODIR%\NetShare.out 2>&1
)
rem net share > %SYSINFODIR%\NetShare.out 2>&1
...
REM Get MFDSUSER and MFDSPASS
set MFDSUSER=
if NOT %2"" == "" (
  call :TEE2 Using mfds user provided on command line: %2
  set MFDSUSER=%2
) else (
  call :TEE2 No MFDSUSER provided on command line
)
set MFDSPASS=
if NOT %3"" == "" (
  call :TEE2 Using mfds password provided on command line
  set MFDSPASS=%3
) else (
  call :TEE2 No MFDS password provided on command line
)
...
call :GetCasDump
rem if "%RUNCASDUMP%"=="1" ( 
rem     if "%SERVER_STATUS%"=="Started" (
rem       call :TEE1 Region %REGION% is started - attempting to get a casdump 
rem       ) else (
rem       call :TEE1 %REGION% region not started or status not detected - attempting casdump anyway
rem       )
rem     call :GetCasDump
rem   ) ELSE (
rem     call :TEE1 Region [%REGION%] not currently running - cannot take casdump
rem   )
...
if "%NO_SYSDIR%"=="1" goto SKIP_COPY_WA
...
REM REM TEST 
REM REM TEST call :TEE2 Reseting MFDS_cfg file to 5.1 version
REM REM TEST SET MFDSCFGFILE=D:\Users\GeoffC\Documents\MFESdiags\Matt\MFDS_cfg.out
REM REM TEST call :TEE2 %MFDSCFGFILE%
REM REM TEST
...
REM don't do the following check if JOURNAL_LOC is null. We do an if exist before the actual copy attempt anyway
if NOT "%JOURNAL_LOC%" == "" (
  REM Stip off any trailing slash and convert to short path
...
REM ----------------------------------------------------------------------------
REM Copy all ESCWA info if available (v5 and later ship escwa)
REM ----------------------------------------------------------------------------
call :TEE1 Copying ESCWA info
for /F "tokens=1,2*" %%g in ('reg query "HKLM\Software\Micro Focus\%FULL_VERSION%" /v ALLUSERSPROFILEPRODUCT') do (set PROD_BASE_DIR=%%i)
call :TEE2 %DATE%,%TIME%, %PROD_BASE_DIR%
call :TEE2 %DATE%,%TIME%, xcopy "%PROD_BASE_DIR%\escwa" %SNAPDIR%\escwa /s/c/Y/i/q
if exist "%PROD_BASE_DIR%\escwa" xcopy "%PROD_BASE_DIR%\escwa\." %SNAPDIR%\escwa /s/c/Y/i/q >>%SNAPLOG% 2>&1
...
REM
REM    14Feb2020 HCL/UVE Start of Changes
REM  Add call to cmd file to gather special diagnostics collection for MFIMS
@echo off
if exist customdiags.cmd call customdiags.cmd %REGION%
echo. 
REM    14Feb2020 HCL/UVE End of Changes
REM
  call :TEE2 REGION=[%REGION%]
) else (
...
REM Remove any leading spaces from REGION
for /f "tokens=* delims= " %%a in ("%REGION%") do set REGION=%%a
call :TEE2  REGION leading spaces removed [%REGION%]
...
IF NOT EXIST "%REGION_WORKAREA%" (
  REM only one additional attempt with a different default
  REM if NO_SYSDIR==2 goto EndGWA
  call :TEE2 no sysdir found, settting NO_SYSDIR=1 to skip WorkArea copy
  set NO_SYSDIR=1
...
REM ============================================================================
set "psCommand=powershell -Command "$pword = read-host 'Enter MFDS Password' -AsSecureString ; $BSTR=[System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($pword); [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)""
...
call :TEE2 Supplied MFDS User : %MFDSUSER% 
rem call :TEE2 %MFDSPASS% 

if NOT %MFDSUSER%"" == "" (
  call :TEE2 Invoke MDUMP with user
  "%MDUMP_EXE%" -a localhost -n %REGION% -u "CN=%MFDSUSER%" -x %MFDSPASS% > %MDUMP_FILE% 2>&1
) else (
  "%MDUMP_EXE%" -a localhost -n %REGION% > %MDUMP_FILE% 2>&1
)
set MDUMP_ERRORLEVEL=%ERRORLEVEL%
type %MDUMP_FILE% | findstr "AUTH_METHOD_NOT_SUPPORTED" >> %SNAPLOG%
set MDUMPAUTHERR=%ERRORLEVEL%

if %MDUMP_ERRORLEVEL% NEQ 0 (
    if %MDUMPAUTHERR% EQU 0 (	
        call :TEE1 Point 1
	    REM ***** Need to get user and password and retry
        call :TEE1 MFDS denied access. Need User and Password for ESCWA/MFDS
        set /p MFDSUSER=Enter MFDS Username: 
        for /f "usebackq delims=" %%p in (`%psCommand%`) do set mfdspass=%%p
    ) 
)
...
echo The diagnostic files will be collected under %%TEMP%%\MFESDIAGDIR (or %%DIAGTEMP%%\MFESDIAGDIR if set).
...
:Get_MFDBFH_CONFIG
SETLOCAL
call :TEE2 Get_MFDDFH_CONFIG

REM see if set in region config
REM Relies on 'mdump' having been run already to generate the output file
REM (NO_MFDS=1 if MFDS not running)
REM if %NO_MFDS%==1 echo can't get region's MFDBFH location - skipping...
if %NO_MFDS%==1 goto ChkOSMFDBFH

REM See if MFDBFH is set in the region's environment and if so return its value (in arg 1) - need name of variable and its length
Call :TEE2 check for MFDBFH in region's environment
call :GetVarFromFile MFDBFH_FILE MFDBFH_CONFIG 13 %MDUMP_ENV_FILE%
if "%MFDBFH_FILE%"=="" call :TEE2 MFDBFH_CONFIG not set in region & goto ChkOSMFDBFH
if NOT "%MFDBFH_FILE%"==""  call :TEE2 MFDBFH_FILE=[%MFDBFH_FILE%] & call :CheckForDollar MFDBFH_FILE 
if EXIST "%MFDBFH_FILE%" (
  call :TEE1 Copying region MFDBFH file: "%MFDBFH_FILE%" 
  copy "%MFDBFH_FILE%" %SNAPDIR% >> %SNAPLOG%
  ) else (
  call :TEE1 MFDBFH set in region but file doesn't exist [%MFDBFH_FILE%]
  )
REM Although the region config may not be valid, since MFDBFH_CFG was actually set that should override the OS env setting...
goto skipOSMFDBFH

:ChkOSMFDBFH
REM If not set in region, check/get from OS environment
Call :TEE2 Check for MFDBFH in OS environment
if "%MFDBFH%"=="" call :TEE2 MFDBFH not set in OS & goto skipOSMFDBFH
if NOT "%MFDBFH%"==""  call :TEE2 MFDBFH=[%MFDBFH%]
if EXIST "%MFDBFH%" (
  call :TEE1 copying OS MFDBFH config file: "%MFDBFH%" 
  copy "%MFDBFH%" %SNAPDIR% >> %SNAPLOG%
  ) else (
  call :TEE1 MFDBFH set in OS but file doesn't exist [%MFDBFH%]
  )
...
REM if running under docker we will suppress running explorer 
if "%MFES_DOCKER%" EQU "1" (
    set MFES_NOPROMPT_SAVE=%MFES_NOPROMPT%
    set MFES_NOPROMPT=1
)
...
if "%MFES_DOCKER%" EQU "1" (
    set MFES_NOPROMPT=%MFES_NOPROMPT_SAVE%
)
...
call :TEE2 CScript //T:%ZIPTIMEOUT% %ZIPITVBS%  %SNAPDIR%  %SNAPDIR%.zip  
...
if "%MFES_DOCKER%" EQU "1" (
powershell -noninteractive -command "$ProgressPreference = 'SilentlyContinue' ; Compress-Archive -Path %SNAPDIR% -DestinationPath %SNAPDIR%" > "%SNAPDIR%\..\MFESDIAGS-cScript-output.log" 2>&1
) ELSE (
CScript //T:%ZIPTIMEOUT% %ZIPITVBS%  %SNAPDIR%  %SNAPDIR%.zip  > "%SNAPDIR%\..\MFESDIAGS-cScript-output.log" 2>&1
)
